...
import { getAssignmentFolder, getAlreadyDownloadedSubmissions } from "../../../lib/pathutils"
...
export const submissionCloneAll = () => {
  return async (dispatch, getState) => {
    ...
    const alreadyDownloadedSubmissions = await getAlreadyDownloadedSubmissions(basePath, assignmentName)
    const selectedSubmissions = selected(getState())
    return Promise.map(selectedSubmissions, submission => {
      return dispatch(submissionClone(submission, cloneDirectory, alreadyDownloadedSubmissions))
    },
